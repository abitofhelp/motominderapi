...
// TestApi_GetMotorcycle verifies a successful response after getting a motorcycle.
func TestApi_GetMotorcycle(t *testing.T) {
	...
	// Insert a motorcycle into the repository so it can be retrieved.
	...
	// ACT
	resp, err := GetMotorcycle(ourApi, insertionViewModel.ID)

	// ASSERT
	assert.True(t, resp.StatusCode == 200)
}

// GetMotorcycle retrieves a motorcycle from the repository using the RESTful API.
// Returns (*response, nil) on success, otherwise (nil, error).
func GetMotorcycle(ourApi *Api, id typedef.ID) (*http.Response, error) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ourApi.GetMotorcycleHandler(w, r, httprouter.Params{httprouter.Param{
			Key:   "id",
			Value: idText,
		}})
	})
