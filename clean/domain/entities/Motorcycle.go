...
func (m Motorcycle) Validate() error {
	return validation.ValidateStruct(&m,
		...
		validation.Field(&m.Make, validation.Required, validation.Length(constants.MinMakeLength, constants.MaxMakeLength), validation.By(IsInvalidManufacturer)),
		// Model cannot be nil, cannot be empty, and max length of 20
		validation.Field(&m.Model, validation.Required, validation.Length(constants.MinModelLength, constants.MaxModelLength)),
		// Year must be between 1999 and 2020, inclusive.
		validation.Field(&m.Year, validation.Required, validation.Min(constants.MinYear), validation.Max(constants.MaxYear)),
